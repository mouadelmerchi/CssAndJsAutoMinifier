var StylePanel = {
    opt: {
        themeStyle: "#mainStyle",
        stylePanel: ".style-panel",
        stylePanelButton: ".style-panel__button",
        cookieColor: "cookieColor"
    }, loadCssFile: "color-panel-style.min.css", init: function (a) {
        if (!this._inited) {
            this._inited = !0, a && (a.on && ($(this).on(a.on), delete a.on), $.extend(this, a));
            var b = this, c = this.getPanel(), d = !1;
            if (!(c.length < 1)) {
                var e = !0;
                if (this.loadCssFile) {
                    var f = this.getCssFileUrl(this.loadCssFile);
                    $('link[href^="' + f + '"]').length < 1 && (e = !1, $("<link>").attr("type", "text/css").attr("rel", "stylesheet").attr("href", f).insertAfter(this.getMainStyleLinkEl()).on("load", function () {
                        c.fadeIn("slow")
                    }))
                }
                c.find(this.opt.stylePanelButton).on("click", function () {
                    c.animate({left: d ? "-" + c.outerWidth() + "px" : 0}, 500), d = !d
        ...
        var c = "?tc=c" + (new Date).getTime(),
            d = a ? this.getCssFileUrl("main-skin-" + a + ".min.css" + c) : this._defaultUrl;
        if (d != b.attr("href")) {
            {
                var e = this;
            ...
            if (f.filter(".active").removeClass("active"), f.filter('[data-value="' + a + '"]').addClass("active"), this._secondCheck) this.checkResetBtnState(); else {
                var e = this;
                setTimeout(function () {
        ...
        Cookies.set(this.opt.cookieColor, a)
    }, getCookie: function () {
        return Cookies.get(this.opt.cookieColor)
    }
};
jQuery(function () {
    StylePanel.init({
